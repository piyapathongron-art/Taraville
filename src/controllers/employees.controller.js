import { allEmployee, createEmployees, deleteEmployeeService, editEmployeesService, getEmployeeByIdService, getEmployeeBySearchService } from "../services/employee.service.js";
...
export async function getEmployeeBySearch(req, res, next) {
    const { search, department, page = 1, limit = 8 } = req.query
    const result = await getEmployeeBySearchService(search, department, parseInt(page), parseInt(limit))
    res.json({
        totalEmployee: result.total,
